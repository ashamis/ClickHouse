...
IcebergMetadata::IcebergFiles IcebergMetadata::getFiles(ContextPtr local_context) const
{
    ...
    auto add_files_from_manifest = [&](const ManifestFilePtr & manifest_file, FileContentType content_type)
    {
        ...
        for (const auto & file : files)
        {
            ...
            record.content_type = static_cast<Int8>(content_type);
            ...
            record.sequence_number = file->added_sequence_number;
...

            {
                ...
                for (size_t j = 0; j < file->common_partition_specification.size() && j < file->partition_key_value.size(); ++j)
                {
                    if (!partition_str.empty())
                        partition_str += ", ";
                    partition_str += file->common_partition_specification[j].partition_name;
                    partition_str += "=";
                    partition_str += file->partition_key_value[j].dump();
                }
                record.partition = std::move(partition_str);
            }
