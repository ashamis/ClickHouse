...
def test_system_iceberg_files(
    ...
    distinct_content = instance.query(
        f"SELECT DISTINCT content FROM system.iceberg_files WHERE database = 'default' AND table = '{TABLE_NAME}'"
    ).strip()
    assert distinct_content == "DATA", f"Expected content 'DATA', got '{distinct_content}'"
    ...
    # Verify added_sequence_number is positive
    min_seq = int(
        instance.query(
            f"SELECT min(added_sequence_number) FROM system.iceberg_files WHERE database = 'default' AND table = '{TABLE_NAME}'"
        )
    )
    if format_version == "2":
